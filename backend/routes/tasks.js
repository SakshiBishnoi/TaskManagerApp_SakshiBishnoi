...
const createBulkNotification = async (userId, assigneeId, taskCount) => {
  try {
    ...
    const notification = new Notification({
      user: userId,        // The person receiving the tasks (receiver)
      assignee: assigneeId, // The person assigning the tasks (sender)
...
router.post('/', async (req, res) => {
  try {
    ...
    if (assignedTo && assignedTo !== req.user.id) {
      ...
      // Only emit socket event if io exists
...
// Add this route to existing tasks routes
