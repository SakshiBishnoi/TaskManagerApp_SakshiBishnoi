...
function TaskCard({ task, onEdit, onDelete, onStatusChange, onChecklistItemToggle, users, isExpanded, onExpand, currentUser }) {
  ...
  // Add getPriorityColor function
  const getPriorityColor = (priority) => {
    switch (priority.toLowerCase()) {
      case 'moderate':
        return '#18B0FF'; // Updated blue
      case 'high':
        return '#FF2473'; // Updated red
      case 'low':
        return '#63C05B'; // Updated green
      default:
        return '#787486'; // Default gray
    }
  };
  ...
  const getAssignedUserEmail = (assignedTo) => {
    ...
    // First try to get user from the populated assignedTo field
    ...
    // If not populated, try to find user in users array
    const user = users.find(u => u._id === assignedTo);
    if (!user) return 'UN';
    
    // Prefer name over email for initials
    if (user.name) {
  ...
  const shouldShowAssignedUser = () => {
    ...
    // Return false if current user is the assigned user
          ...
          <div className="checklist-items">
            {task.checklist.map((item, index) => (
              <div key={index} className="checklist-item">
                ...
                />
                <span 
                  title={item.text} // Native HTML tooltip
                  data-text={item.text}
                >
                  {item.text}
                </span>
